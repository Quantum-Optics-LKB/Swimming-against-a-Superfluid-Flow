...
def poincare_sum_grad(angles: np.ndarray, r: int = 1) -> np.ndarray:
    """
    Compute the Poincaré index around a plaquette of radius r.

    Args:
        angles (np.ndarray): Derivative of the polar angles of the vector field, shape (2, :, :).
        r (int): Radius of the plaquette.

    Returns:
        np.ndarray: The computed Poincaré index.
    """
    grad_y = angles[0]  # Première composante : dérivée selon y
    grad_x = angles[1]  # Deuxième composante : dérivée selon x

    Ny, Nx = angles.shape[1], angles.shape[2]  # Taille de la grille
    cont = np.zeros((Ny, Nx), dtype=np.float32)
    ...
    for i in numba.prange(Ny):
        for j in range(Nx):
            ...
            for k in range(0, r):
                dphi = grad_y[i, (j + k) % Nx]
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = grad_x[(i + k) % Ny, (j + r) % Nx]
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = -grad_y[(i + r) % Ny, (j + r - k - 1) % Nx]
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = -grad_x[(i + r - k - 1) % Ny, j]
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

            # Normalisation finale
            cont[ii, jj] = poincare / (2 * np.pi)
    return cont
...
def poincare_sum(angles: np.ndarray, r: int = 1) -> np.ndarray:
    """
    Compute the Poincaré index around a plaquette of radius r.

    Args:
        angles (np.ndarray): Polar angles of the vector field (2D array).
        r (int): Radius of the plaquette.

    Returns:
        np.ndarray: The computed Poincaré index.
    """
    cont = np.zeros((angles.shape[0], angles.shape[1]), dtype=np.float32)
    for i in numba.prange(angles.shape[0]):
        for j in range(angles.shape[1]):
            ii = (i + r // 2) % angles.shape[0]
            jj = (j + r // 2) % angles.shape[1]
            poincare = 0.0

            for k in range(0, r):
                # Direct angle differences along the square contour
                dphi = (
                    angles[i, (j + k + 1) % angles.shape[1]]
                    - angles[i, (j + k) % angles.shape[1]]
                )
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = (
                    angles[(i + k + 1) % angles.shape[0], (j + r) % angles.shape[1]]
                    - angles[(i + k) % angles.shape[0], (j + r) % angles.shape[1]]
                )
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = (
                    angles[(i + r) % angles.shape[0], (j + r - k - 1) % angles.shape[1]]
                    - angles[(i + r) % angles.shape[0], (j + r - k) % angles.shape[1]]
                )
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi

                dphi = (
                    angles[(i + r - k - 1) % angles.shape[0], j]
                    - angles[(i + r - k) % angles.shape[0], j]
                )
                dphi = (dphi + np.pi) % (2 * np.pi) - np.pi
                poincare += dphi
...
def helmholtz_decomp(field: np.ndarray, plot=False, dx: float = 1) -> tuple:
    ...
    if plot:
        ...
        im1 = ax[0, 1].imshow(np.hypot(v_inc[0], v_inc[1]))
        ax[0, 1].set_title(r"$|v^{inc}|$")
        ax[0, 1].set_xlabel("x")
        ax[0, 1].set_ylabel("y")
        fig.colorbar(im1, ax=ax[0, 1])
        ...
        ax[1, 1].streamplot(
            XX, YY, velo[0], velo[1], density=2, color="white", linewidth=0.5
        )
        ax[1, 1].set_title(r"$v^{tot}$")
        ax[1, 1].set_xlabel("x")
        ax[1, 1].set_ylabel("y")
        fig.colorbar(im3, ax=ax[1, 1], label=r"$|v^{tot}|$")
        plt.show()

    return velo, v_inc, v_comp


def helmholtz_decomp_phase(field: np.ndarray, plot=False, dx: float = 1) -> tuple:
    """Decomposes a phase picture into compressible and incompressible velocities
...
def vortex_detection(phase: np.ndarray, plot: bool = False, r: int = 1) -> np.ndarray:
    ...
    if plot:
        if windings.ndim == 3:
            windings = windings.mean(axis=0)
        fig, ax = plt.subplots(1, 2, figsize=[8, 4])
        im0 = ax[0].imshow(phase, cmap="twilight_shifted", interpolation="none")
        im1 = ax[1].imshow(
            windings, cmap="seismic", norm=colors.CenteredNorm(vcenter=0)
        )
        ax[0].scatter(vortices[:, 0], vortices[:, 1], c=vortices[:, 2], cmap="bwr")
        fig.colorbar(im0, ax=ax[0], shrink=0.5, label="Phase")
        fig.colorbar(im1, ax=ax[1], shrink=0.5, label="Winding")
        plt.show()
    return vortices


def vortex_detection_schiebler(phase: np.ndarray, plot: bool = False) -> np.ndarray:
    velo = velocity(phase)
    vy, vx = velo[1], velo[0]
    ...
    y = signal.convolve2d(vy, ky, mode="same")
    x = signal.convolve2d(vx, kx, mode="same")
    windings = x + y

    detected = (windings < -2 * np.pi) | (2 * np.pi < windings)

    if plot:
        plt.figure(figsize=(8, 4))
        plt.imshow(windings, cmap="seismic", norm=colors.CenteredNorm(vcenter=0))
        plt.colorbar(label="Winding")
        plt.show()

    return np.argwhere(detected)


def sell_detection(phase: np.ndarray, plot: bool = False, r: int = 1) -> np.ndarray:
    """Detects the sell points positions using velo polar angles
    ...
    """
    velo = velocity(phase)
    angle = np.arctan2(velo[0], velo[1])
    # dangle = velocity(angle)
    if r == 1:
        poincare = poincare_sum(angle, r)
        # poincare = poincare_sum_grad(dangle, r)
        cond_plus = poincare >= 1
        cond_minus = poincare <= -1
    else:
        poincare = np.zeros((r, phase.shape[0], phase.shape[1]), dtype=np.float32)
        for ir in range(r):
            poincare[ir, :, :] = poincare_sum(angle, ir + 1)
            # poincare[ir, :, :] = poincare_sum_grad(dangle, ir + 1)
        poincare_mean = np.mean(poincare, axis=0)
        cond_plus = poincare_mean >= 1
        cond_minus = poincare_mean <= -1

    print("Poincare shape:", poincare.shape)
    print("cond_plus sum:", np.sum(cond_plus))
    print("cond_minus sum:", np.sum(cond_minus))
    plus_y, plus_x = np.where(cond_plus)
    minus_y, minus_x = np.where(cond_minus)
    vortices = np.zeros((len(plus_x) + len(minus_x), 3), dtype=np.float32)
    vortices[0 : len(plus_x), 0] = plus_x
    vortices[0 : len(plus_x), 1] = plus_y
    vortices[0 : len(plus_x), 2] = 1.0
    vortices[len(plus_x) :, 0] = minus_x
    vortices[len(plus_x) :, 1] = minus_y
    vortices[len(plus_x) :, 2] = -1.0
    if plot:
        if poincare.ndim == 3:
            poincare = poincare.mean(axis=0)
        fig, ax = plt.subplots(1, 2, figsize=[8, 4])
        im0 = ax[0].imshow(angle, cmap="twilight_shifted", interpolation="none")
        im1 = ax[1].imshow(
            poincare, cmap="seismic", norm=colors.CenteredNorm(vcenter=0)
        )
        ax[0].scatter(vortices[:, 0], vortices[:, 1], c=vortices[:, 2], cmap="Greens")
        fig.colorbar(im0, ax=ax[0], shrink=0.5, label="Angle")
        fig.colorbar(im1, ax=ax[1], shrink=0.5, label="Saddle and node")
        plt.show()
    return vortices
...
def cluster_vortices(vortices: np.ndarray) -> list:
    ...
    assert 2 * len(dipoles) + 2 * pairs.shape[0] + len(queue) == vortices.shape[0], (
        "PROBLEM count"
    )
